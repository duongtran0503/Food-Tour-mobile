...
export default function HomeScreen() {
  const { t, router, restaurants, categoryData } = useHome();
  
  return (
    <View className="flex-1 bg-white">
      <StatusBar barStyle="light-content" />
      <View className="h-64">
        <ImageBackground source={require('@/assets/images/banner.png')} className="flex-1" resizeMode="cover">
          <View className="flex-1 bg-black/20 pb-12"> 
            <SafeAreaView edges={['top']}>
              <View className="flex-row items-center justify-between px-6 py-4">
                <View>
                  <Text className="text-white/90 text-xs font-bold uppercase tracking-widest shadow-sm">{t('screens.home.welcome')}</Text>
                  <Text className="text-4xl font-black text-white italic shadow-lg">FoodVK</Text>
                </View>
      ...
      <View className="flex-1 bg-white rounded-t-[40px] -mt-8 shadow-2xl overflow-hidden">
        <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 100, paddingTop: 20 }}>
          <View className="mb-4">
            <Text className="px-4 text-xl font-extrabold text-slate-800 mb-5">{t('screens.home.categories')}</Text>
          ...
          <View className="mb-4">
            <View className="flex-row items-center justify-between px-4 py-2 mb-2">
              <View className="flex-row items-center">
                ...
                <Text className="ml-2 font-black text-lg text-slate-900">{t('screens.home.popular_restaurants')}</Text>
              </View>
              <TouchableOpacity onPress={() => router.push("/restaurant/all")}>
                <Text className="text-[#930004] font-bold text-sm">{t('screens.home.view_all')}</Text>
              </TouchableOpacity>
            </View>
            <ScrollView horizontal showsHorizontalScrollIndicator={false} className="pl-2">
