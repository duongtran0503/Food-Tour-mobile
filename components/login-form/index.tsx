...
export default function LoginForm() {
  ...
  return (
    <View className="gap-y-5">
        ...
        <View className={`flex-row items-center bg-slate-50 border ${errors.email ? 'border-red-500' : 'border-slate-100'} rounded-2xl px-4 h-16`}>
          ...
          <Controller
            ...
            render={({ field: { onChange, onBlur, value } }) => (
              <TextInput 
                placeholder={t('screens.login.email_placeholder')} // 3. Dịch placeholder
      ...
      <View>
        <Text className="text-slate-500 font-bold ml-1 mb-2">
          {t('screens.login.password_label')}
        </Text>
      ...
      <TouchableOpacity className="self-end">
        <Text className="text-primary font-bold">
          {t('screens.login.forgot_password')}
        </Text>
      </TouchableOpacity>
